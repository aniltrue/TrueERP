...
$sql = "SELECT * FROM Bill ORDER BY BillYear, BillDate ASC";
$rslt = $conn->query($sql);
$conn->close();

if($rslt->num_rows == 0) {
	echo '<p class="w3-center w3-text-red">Kayıtlı fatura bulunamadı.</p>';
	include('tail.php');
	exit;
}
...
<tr class="w3-teal">
  ...
  <th>Tarih</th>
  <th>Dönem</th>
  <th class="noExl">Faturayı Güncelle</th>
</tr>
...
foreach($Rows as $row) {
	...
	echo '<td>' . $row["BillDescription"] . '</td>';	
	echo '<td>' . $row["BillDate"] . '</td>';	
	echo '<td>' . $row["BillYear"] . '</td>';	
	echo '<td class="noExl"><a class="w3-btn w3-teal w3-round-xlarge" href="UpdateBill.php?BillID=' . $row["BillID"] . '">Güncelle</a></td>';
	echo '</tr>';
}
